...
class LuaAsyncContextRegistry {
    public:
        ...
        static void set(lua_State* L, const std::shared_ptr<T>& context) {
            if (!L) throw std::invalid_argument("LuaAsyncContextRegistry::set received null Lua state");

            if (!context) throw std::invalid_argument("LuaAsyncContextRegistry::set received null context");
        ...
        static std::shared_ptr<T> get(lua_State* L) {
            ...
            if (it == contexts<T>().end()) return nullptr;
