...
void LuaRoutes::registerRoute(const std::string& path, drogon::HttpMethod method, const luabridge::LuaRef& handler) {
    ...
    app().registerHandler(path, [handler](const drogon::HttpRequestPtr& req, std::function<void(const drogon::HttpResponsePtr&)>&& callback) {
            try {
                Json::Value json = LuaRoutes::executeHandler(handler);
                LuaRoutes::sendJsonResponse(json, std::move(callback));
            }
            catch (const std::exception& e) { LuaRoutes::sendErrorResponse(e.what(), std::move(callback)); }

        },
        {method});
}

Json::Value LuaRoutes::executeHandler(const luabridge::LuaRef& handler) {
    if (handler.isTable())
        return executeLuaTable(handler);

    if (handler.isFunction())
        return executeLuaFunction(handler);
...
Json::Value LuaRoutes::executeLuaFunction(const luabridge::LuaRef& handler) {
    lua_State* L = handler.state();
    const int base = lua_gettop(L);

    // Push and call Lua function.
    handler.push(L);
